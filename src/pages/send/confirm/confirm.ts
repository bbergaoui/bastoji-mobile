...
export class ConfirmPage {
  ...
  ionViewWillEnter() {
    ...
    try {
      ...
      var message = this.translate.instant(
        'Copay only supports Bastoji Cash using new version numbers addresses'
      );
  ...
  private showSendMaxWarning(wallet, sendMaxInfo): Promise<any> {
    return new Promise(resolve => {
      ...
      let msg = this.replaceParametersProvider.replace(
        this.translate.instant(
          '{{fee}} {{coin}} will be deducted for bastoji networking fees.'
        ),
  ...
  public approve(tx, wallet): Promise<void> {
    ...
    if (this.paymentExpired) {
      this.popupProvider.ionicAlert(
        null,
        this.translate.instant('This bastoji payment request has expired.')
      );
      return undefined;
    }
