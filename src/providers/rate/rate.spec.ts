...
describe('RateProvider', () => {
  ...
  const btcResponse = [
    { code: 'BTJ', name: 'Bastoji', rate: 1 },
    { code: 'USD', name: 'US Dollar', rate: 11535.74 },
    { code: 'BCH', name: 'Bastoji Cash', rate: 7.65734 }
  ];
  const bchResponse = [
    { code: 'BTJ', name: 'Bastoji', rate: 0.130377 },
    { code: 'USD', name: 'US Dollar', rate: 1503.3 },
    { code: 'BCH', name: 'Bastoji Cash', rate: 1 }
  ];
  ...
  it('should get BTJ rates', () => {
    service.updateRatesBtc().then(() => {
      expect(service.isBtcAvailable()).toBe(true);
      expect(service.getRate('BTJ')).toEqual(1);
  ...
  it('should get BCH rates', () => {
    service.updateRatesBch().then(() => {
      ...
      expect(service.getRate('BTJ', 'bch')).toEqual(0.130377);
  ...
  it('should covert BTJ satoshis to fiat', () => {
    // before we have rates
  ...
  it('should covert fiat to BTJ satoshis', () => {
    // before we have rates
  ...
  it('should list alternatives', () => {
    ...
    service.updateRatesBtc().then(() => {
      expect(service.isBtcAvailable()).toBe(true);
      expect(service.listAlternatives(false)).toEqual([
        { name: 'Bastoji', isoCode: 'BTJ' },
        { name: 'US Dollar', isoCode: 'USD' },
        { name: 'Bastoji Cash', isoCode: 'BCH' }
      ]);
      expect(service.listAlternatives(true)).toEqual([
        { name: 'Bastoji', isoCode: 'BTJ' },
        { name: 'Bastoji Cash', isoCode: 'BCH' },
