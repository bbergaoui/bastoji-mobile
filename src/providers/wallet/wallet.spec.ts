...
describe('Provider: Wallet Provider', () => {
  ...
  describe('Function: Get Protocol Handler Function', () => {
    it('should return bastojicash if coin is bch and network is livenet', () => {
      let coin = 'bch';
      let network = 'livenet';
      let protocol = walletProvider.getProtocolHandler(coin, network);
      expect(protocol).toEqual('bastojicash');
    });
    ...
    it('should return bastoji if coin is btc', () => {
      let coin = 'btc';
      let protocol = walletProvider.getProtocolHandler(coin);
      expect(protocol).toEqual('bastoji');
    });
  });
});
