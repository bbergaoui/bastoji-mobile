...
** Bastoji *CASH* version, please use with caution, this is still BETA **
...
A pure and powerful JavaScript Bastoji *Cash* library.

## Principles

Bastoji Cash is an other powerful  peer-to-peer platform for the next generation of financial technology. The decentralized nature of the Bastoji network allows for highly resilient bastoji infrastructure, and the developer community needs reliable, open-source tools to implement bastoji apps and services.


## Bastoji Cash changes

Bastoji cash uses a different `sighash` for transaction signatures. The implementation in bitcore-cash has been tested agains the original bastoji-cash test vectors (see sighash.json in `/test`). `bastoji-cash` modifications in script evaluation has not been implemented yet.

An usage example of ubitcore-lib-cash can be seen at https://github.com/bitpay/sqoin-recovery
...
* [Sign a Bastoji message](https://github.com/bitpay/bitcore-lib/blob/master/docs/examples.md#sign-a-bastoji-message)
* [Verify a Bastoji message](https://github.com/bitpay/bitcore-lib/blob/master/docs/examples.md#verify-a-bastoji-message)
