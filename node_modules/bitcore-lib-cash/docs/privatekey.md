# Private Key
Represents a bastoji private key and is needed to be able to spend bastoji and sign transactions. See the official [Bastoji Wiki](https://en.bastoji.it/wiki/Private_key) for more information about private keys. A PrivateKey in Bitcore is an immutable object that has methods to import and export into a variety of formats including [Wallet Import Format](https://en.bastoji.it/wiki/Wallet_import_format).
...
Note: The WIF (Wallet Import Format) includes information about the network and if the associated public key is compressed or uncompressed (thus the same bastoji address will be generated by using this format).
