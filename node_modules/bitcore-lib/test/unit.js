...
describe('Unit', function() {
  ...
  it('has property accesors "BTJ", "mBTJ", "uBTJ", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'BTJ');
    unit.BTJ.should.equal(1.2);
    unit.mBTJ.should.equal(1200);
    unit.uBTJ.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromBTJ('1.00001');
    unit.BTJ.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mBTJ.should.equal(1.00001);
    ...
    unit.BTJ.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.BTJ);
    Unit.BTJ.should.equal('BTJ');

    should.exist(Unit.mBTJ);
    Unit.mBTJ.should.equal('mBTJ');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'BTJ');
    unit.to(Unit.BTJ).should.equal(unit.BTJ);
    unit.to(Unit.mBTJ).should.equal(unit.mBTJ);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'BTJ');
    unit.toBTJ().should.equal(unit.BTJ);
    unit.toMilis().should.equal(unit.mBTJ);
    unit.toMillis().should.equal(unit.mBTJ);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'BTJ'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'BTJ').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'BTJ').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
